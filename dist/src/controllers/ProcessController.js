...
export default {
    ...
    async getProcess(req, res) {
        try {
            ...
            const [professorEnrollments, studentEnrollments, users, classes, subjects, schoolPeriod] = await Promise.all([
    ...
    async getAllProcesses(_req, res) {
        try {
            ...
            const processesWithPeriods = await Promise.all(processDocs.map(async (process) => {
    ...
    */
    async updateProcessPart(req, res) {
        try {
            const { processId, model } = req.params;
            const updateData = req.body;
            if (!processId || !model) {
            ...
            if (model === 'Subject' && updateData.code) {
                for (const doc of existingDocs) {
                    await mongoose.model('Class').updateMany({ subjectCode: doc.code, processId }, { subjectCode: updateData.code });
                }
            }
            const updatedProcess = await mongoose.model('Process').findOneAndUpdate({ processId }, updateData, { new: true, runValidators: true });
            if (!updatedProcess) {
                return res.status(404).json({ error: 'Process not found' });
            }
    ...
    async updateStatus(req, res) {
        try {
            const { processId } = req.params;
            const updateData = req.body;
            const updatedProcess = await ProcessDocument.findOneAndUpdate({ processId }, updateData, { new: true, runValidators: true });
            if (!updatedProcess) {
                return res.status(404).json({ error: 'Process not found' });
            }
            return res.status(200).json(updatedProcess);
